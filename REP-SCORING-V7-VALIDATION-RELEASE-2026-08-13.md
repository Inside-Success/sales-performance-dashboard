...
The backfill safety contract is prepared but deliberately not activated. Immutable scorer-version keys, ten-call partitioning, expired-lease recovery, deterministic exponential backoff with jitter, four-attempt dead-lettering, and final-only progress accounting are covered by deterministic tests. `n8n/rep-scoring-v7/backfill-control-plane.sql` defines an un-applied Postgres ledger with a unique source/version constraint, atomic `FOR UPDATE SKIP LOCKED` claims, renewable lease tokens, stale-worker protection, independent retry/dead-letter settlement, and separate `live` / `backfill` lanes. Airtable remains the evidence store rather than being misused as an atomic lock.

On explicit approval, activation still requires applying that migration, wiring authoritative active-execution/provider-health inputs and the dispatcher, and running a no-AI lease/duplicate smoke test before admitting calls. The approved coordinator must not reuse the validation launcher's inefficient full V6.3 reference-score scan: that scan read about 145,824 Airtable rows and delayed the extension launcher by roughly 703 seconds even though it was unnecessary to V7 scoring. The full-backfill target remains one to two hours, but speed cannot override organization capacity or safety backpressure.
