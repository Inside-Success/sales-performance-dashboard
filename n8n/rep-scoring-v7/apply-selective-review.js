...
const source = $('Prepare V7 Selective Review').item.json;
...
merged.dimensions = (Array.isArray(merged.dimensions) ? merged.dimensions : []).map((dimension) => ({
  ...
  criteria: (Array.isArray(dimension?.criteria) ? dimension.criteria : []).map((criterion) => {
    ...
    const allowed = new Set(['exceptional', 'strong', 'competent', 'partial', 'weak', 'missed', 'harmful', 'not_applicable', 'not_observable']);
