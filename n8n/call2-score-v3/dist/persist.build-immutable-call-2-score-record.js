...
if (!['magic-mike-call2-evidence-score-v1','magic-mike-call2-evidence-score-v2','magic-mike-call2-evidence-score-v3'].includes(scorerVersion)) return [{json:{route:'not_scored',write_enabled:false,reason:'unsupported_scorer_version'}}];
...
const scoreFields = {
  ...
  'Behaviour Checks JSON':JSON.stringify(Array.isArray(result.procedural_checks) ? result.procedural_checks.map((check) => ({ name:String(check.name||''), label:String(check.label||''), status:String(check.status||''), timestamp:String(check.timestamp||''), speaker:String(check.speaker||''), quote:String(check.quote||''), validation_note:String(check.validation_note||'') })) : []),
  ...
  'Call Context JSON':JSON.stringify({
    ...
    outcome:{ classification:String((result.call_outcome && result.call_outcome.classification) || (score.call_outcome && score.call_outcome.classification) || 'unknown'), reason:String((result.call_outcome && result.call_outcome.reason) || '') },
