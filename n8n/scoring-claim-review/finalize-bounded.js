...
if(s.status==='checked'){
 try{
  const revised=apply(s.raw,s.build,s.audit,s.review),checked=reviewedValidate(revised,s.build)[0].json;
...
const audit={revision:'bounded-claims-2026-09-10',status:s.status,model:'claude-sonnet-4-6',checker_called:s.should_review,reassessment_called:s.needs_reassessment,accepted_corrections:s.status==='passed'?(s.review.decisions||[]).filter(x=>x.accepted).map(x=>x.claim_id):[],syntax_repairs:s.syntax_repairs||0,failure:s.failure||null};
