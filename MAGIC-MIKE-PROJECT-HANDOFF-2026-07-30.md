...
Final cutover result: all 1,660 fixed-cohort source calls reached a terminal V7.1 result. The reconciled evidence contains 1,483 scores and 188 fair terminal exclusions. Earlier calibration retries produced 11 extra stored rows across 10 source IDs; identical retries are collapsed and conflicting retries fail closed in both the manager scorecard and Coaching overlay, with no evidence rows deleted. The score distribution is 12.3–85.4 with an 80.6 median, 428 scores below 75, 206 below 60, and no score at or above 90.

Scheduled production proof passed on coordinator execution `494326`: it selected two post-cutoff live calls, zero historical calls, and dispatched worker execution `494328`, which completed both calls successfully. V7.1 coordinator `gXGkKGtsXPudAePR`, worker `QPUh149BvYlqhKOq`, and Coaching `L8Nn7xncA9ZPDdWA` are active. Preserved V6.3 coordinator `EghbY2jr86yjJl4d` and worker `w8JaLibcm8zqVGP1` are inactive; the one-time continuation and audit workflows are inactive.
...
## September 10 — September-only scoring backfill launched

See MAGIC-MIKE-SEPTEMBER-BACKFILL-2026-09-10.md in sales-performance-dashboard-backfill-2026-09-10. PR #182 is merged and production verified. Frozen queue 292 since September 1 ET, after skipping 3 already-current scores from 295 distinct scored calls. Five native canary calls all passed and persisted, total $0.962697; no duplicate scores. Full queue released with five hosted workers, $81 automatic ceiling, durable paid-result storage and no automatic paid retries. Dispatcher QqO9VopWcGUOXslV, worker iC8zguHy3SbFc122, failure recorder sN4Nry0lnFoNcz5E. Track at /manager/rep-scoring. Old scores remain privately recoverable but are hidden from display; only latest passed reviewed scores from September 1 onward appear. Coaching reports, compliance, source documents, Slack and live scorer configuration are unchanged. Do not seed/reset/restart the run without checking mm_september_runs/mm_september_jobs and active dispatcher executions first.

### September 10 backfill recovery
See `MAGIC-MIKE-SEPTEMBER-BACKFILL-2026-09-10.md` and `n8n/backfill-recovery/README.md`. The 40-minute dispatcher timeout is replaced with completion-driven short dispatches. Reviewer contract repairs reuse paid primary assessments. The five-worker and $81 limits, September cutoff, rep-only numeric display, and compliance boundaries remain in force.
...
## Coaching closing-section duplication fix — September 11, 2026

Annalia report5909 exposed a complete-path formatting regression: the structured formatter copied outcome and strengths into the paid closing field; downstream sentence repair collapsed blank lines, defeating paragraph-only display deduplication. The repaired presentation contract now parses numbered observations before paragraph boundaries, preserves unique closing advice and outcome evidence, and prevents hidden closing copies from suppressing buyer concerns. The structured formatter no longer builds the paid closing field by appending strengths. No schema, AI model/prompt, scoring or compliance policy change.

Published official workflow a87f0491-9f86-446b-a0d0-ba4e12f952d8 and manual workflow f6c5e17b-ada1-40c8-b323-244b3eb9684e. Changed only existing coaching formatter library copies and shared Slack/Google-report rendering code. Fresh ownership verified Moonis project Q4yRpcI5DdgazjAh; credentials/connections and all other nodes remain unchanged. Active published source parity checked.
...
### Manager release verification
PR #188 merged as 23b23176fededffa69c3ef8ca94385460b6f39ed; production dpl_BSCScW6BwHcbYzVfJUdxPjDmJvo5 READY. Authenticated live checks confirmed restored July20/July27/Aug3/Aug10 weekly counts 60/99/67/70, full attendance pagination, working Call1/Call2+ and date/search filters, 14/30-day saved links, expandable usage details and complete score evidence, visible interactive sales scatterplot, and removal of the backfill box. The Mark example average remains56.6 and individual scores unchanged. Unauthenticated manager routes redirect to sign-in. Desktop and narrow-screen scorecard checked.

A follow-up display guard rejects obvious historical client-name extraction fragments (e.g. “who engaged” and “through the license video”); optional names never prevent score access. Four client-label tests pass, bringing scoped regression coverage to21 tests. Scoped lint and webpack production build passed. No local dev server, AI calls, scoring changes, Slack sends or source data writes. Preview provisioning failed before build (existing platform preview issue); successful production build and live checks provide release evidence.

## Tyler manager clarity revision — September 12

Authorized by Moonis after Tyler's feedback in C0B402VSESY. Rep usage now answers who opens their own coaching: three summary figures, one searchable/filterable rep table, and 7/30/all-time report-availability cohorts. Reports are grouped by dashboard created_at; an owner's open through the snapshot time clears that report. Other viewers cannot clear it. Current roster remains official call activity within30days. Complete roster/report counts replace75/25 display limits. No-open reports older than48hours are follow-up candidates, not an assertion the rep never read Slack. Last coaching opened is the latest recorded official-report open; optional details retain engagement and other-rep usage. Diagnostics load only when explicitly requested.

The new SELECT-only usage reader is independent of db.ts legacy analytics, sales-correlation calculations and tracking. No migrations, source writes, schema changes or scorer changes. No-show duplicate executive panel removed; comparison count retained in compact summary. Sales chart remains prominent; duplicated supporting metrics/group details moved into existing expansion. Compliance, scoring and Ask Sales unchanged.

Rollback: production dpl_j8WmHwhEi9jZx98hXL2VbqimNsdq / main9c240ebb282013d2f77ff3a8cc93106d3587077a. Revert scoped UI/new-reader commit; no data rollback required. Main merge deploys Vercel; Ask Sales path-filtered CI is outside this change. Regression tests cover full roster/counts, owner-vs-other activity, deduplication,48-hour boundary, zero-report denominator and period parsing. Production verification to follow release.

### Live verification of Tyler revision
PR190 merged8e17114bb2e75f9db262ace4fae5a25aacfe63ad; production dpl_Gn8WNHzRehoJDC9rFV6Y84KGWWsb READY. Seven-day live row sums match summaries exactly:126 reps,269 available,25 owner opens,112 overdue.30days:1019/110/777; allhistory4310/476/3702 at checkpoint. Allhistory Julianas24available/20owner opens, other71 opens separately. Search and unopened filters combine, details distinguish New vs Over48h, and diagnostics load on request. Sales chart still interactive and $6606 high-vs-low revenue gap unchanged.14day no-show saved link renders167/2822,6%,+20vs147 with existing chart. Unauthenticated usage/no-show/sales routes redirect to sign-in preserving URLs. Runtime error query returned no entries.14 focused tests, lint and production build passed.390px layout inspected; follow-up gives search its own mobile row and hides obvious historical client-name extraction prose without changing source data. No Slack messages, AI calls, data writes, backfill, scoring or compliance changes.

## 2026-09-12 — Rep usage wording clarification

Scoped copy update: explicit assigned-rep report opens, readable “X of Y” totals, denominator explanation, two-day unopened description, short table guidance, and clearer received/opened/latest-open/View details labels. Counting, queries, filters, access controls and other manager pages are unchanged. Baseline/rollback: main c198ec727ff2a028016fdac7cc4e9b6037ad9e4f. Scoped ESLint, eight usage tests and webpack production build passed. Production verification follows release.

## 2026-09-14 — Actionable rep usage insights

Adds opening percentages with report counts, visible 10+ second engagement, four rolling weeks of own-feedback consistency, and never-opened/inactive/regular filters. Current 7/30/all report totals remain unchanged. Last opened now explicitly uses OWN report activity (other-report activity remains in details). Weeks with no newly available report are excluded; regular means all eligible weeks active with at least 3 eligible weeks. Never/inactive require overdue feedback; inactive means last own open over 7 days ago. A separate timely-opening trend compares two equal 7/30-day cohorts ending at least 48 hours ago, counting only opens within the first 48 hours; all-time has no fabricated period comparison.

Read-only query reads existing history in one request for owner history, repeat-open weeks and selected-period totals. No event tracking, schemas, other manager pages, scoring, AI or workflows changed. User clarified reps do not belong to the Slack coaching channel; removed the unnecessary Slack disclaimer. Rollback baseline main 0e23f53 / deployment dpl_GxAhHeN7aP1q3mphgR68PYNHnwf1. Fifteen usage tests, scoped lint and webpack build passed. Live SELECT-only comparisons matched old counts for every period; independent weekly SQL matched all 123 eligible reps. Pre-release 7-day snapshot: 310 reports,33 opened,31 engaged,200 overdue; first48h26/269 vs18/230; filter counts57 never/13 inactive/5 regular. Production UI verification follows release.
