...
- V3 model adapter: evaluator-injected V4 direct DeepSeek provider (`--v3-provider=v4`);
...
- The user-authorized isolated DeepSeek credential is loaded only from a mode-`600` temporary file outside the repository, is never printed or committed, and must be removed after validation.
