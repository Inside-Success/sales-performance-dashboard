...
V6 dimension-first calibration is recorded in `REP-SCORING-V6-CALIBRATION-RELEASE-2026-08-10.md` and exposed only at `/manager/rep-scoring/v6-calibration`. It uses two immutable scorer versions to score the same six Call 1 and six Call 2 calls twice. The funded Aug 10 run completed 12 unique calls and 24 immutable assessments. Nine pairs pass the strict same-band/maximum-10-point stability rule and three are explicitly marked for human review. V6 does not replace the current manager page, start a backfill, or publish scores into Coaching until the owner reviews and approves the calibration.
...
V6 calibration worker: `lfyLHukUvx9887it`. One-time V6 launcher: `KCh1MiSacbFMge5I`. The worker is published but has only an Execute Workflow Trigger; the external one-time launcher is inactive. The launcher checks the dedicated DeepSeek balance before reading candidates or dispatching work. Execution `468131` proved the unavailable-balance gate failed closed. After funding, execution `468673` dispatched the first four workers, and completion execution `468876` safely added a replacement for one two-rep recording that was correctly quarantined in both rounds. The final store has 12 complete pairs; no schedule or backfill is attached.

The V6 launcher selects six Call 1 and six Call 2 calls from immutable V5 shadow results across low, lower-quartile, middle, high, difficult-prospect, and evidence-limitation cases. It dispatches four bounded sequential workers: Call 1 and Call 2 for round 1, then the same calls for round 2. The immutable scorer versions are `rep-reviewer-v6-calibration-r1` and `rep-reviewer-v6-calibration-r2`. The known two-rep source `recumvq2gnbAbB4oe` is excluded from calibration selection because neither rep can be scored fairly from that shared recording; its quarantine history remains intact.
...
V6 dashboard PR `#139` merged as `0ddbe7a721ec66609d6d78d0303ecafa8da8881d`. Production deployment `dpl_9rh9WTqasj4BonuigVwVa6qya7Ta` reached `READY`, owns the canonical alias, and served the protected V6 route without runtime errors. An unauthenticated verification request received the expected Google sign-in page rather than a 404.

The completed production calibration shows scores from `45.0` to `85.0` across four bands from `Needs Improvement` through `Excellent`; it does not reproduce V5's easy 100s. All 24 assessments are gradeable, dimension-evidence verified, and free of validation warnings. Nine of twelve pairs are stable under the strict rule. The three review-marked pairs demonstrate that temperature zero does not make a generative judgment perfectly deterministic; those cases must be reviewed by the owner before any scorer promotion or backfill.
...
V5 shadow coordinator: `uWfyXPrNDzQ2Eixe`. V5 cost-safe worker: `XPFqJlWGRRCiNqDn`. The coordinator is intentionally inactive until the user confirms the dedicated DeepSeek balance has been recharged. When resumed it gets a 15-minute opportunity, checks DeepSeek balance and the latest cost-cap coverage before heavy work, retains the no-overlap lease gate, and admits up to 80 calls per clear slot across eight sequential ten-call workers. The full fixed-date inventory remains 3,328 eligible calls since July 18, 2026, but the current validation target stops at 1,500 finalized calls.
...
The production administrators are exactly `syed.haider@insidesuccess.com`, `tyler@mawercapital.com`, `jawad.saghir@insidesuccess.com`, `raul.rios@mawercapital.com`, `rich.allen@mawercapital.com`, and `mike@insidesuccesstv.com`. All six addresses pass the global Google sign-in domain gate and are separately required by the rep-scoring page allowlist. Preview deployments do not inherit production authentication secrets and are not valid review links. The sign-in page distinguishes an unconfigured preview from a genuine email rejection so infrastructure failures do not appear to be allowlist decisions.
