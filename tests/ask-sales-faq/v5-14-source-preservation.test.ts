...
import { runAskSalesFaqV514, selectV514ActiveDirectPolicy, v514BoundedDirectDecision } from "@/lib/ask-sales-faq/v5-14/runtime";
import { V514_CURRENT_PRICES_AND_PLANS_POLICY } from "@/lib/ask-sales-faq/v5-14/knowledge";
...
describe("Ask Sales V5.14 governed source preservation", () => {
  it("lets a later admin-approved release supersede a bounded direct policy", () => {
    const released = {
      ...V514_CURRENT_PRICES_AND_PLANS_POLICY,
      id: "kr_test_release",
      decision: "The newly approved price rule controls.",
      authority: 118,
      effective_at: "2026-08-01T00:00:00.000Z",
      last_reviewed: "2026-08-01",
      source: {
        ...V514_CURRENT_PRICES_AND_PLANS_POLICY.source,
        kind: "admin_approved_knowledge_release" as const,
      },
    };

    expect(selectV514ActiveDirectPolicy(V514_CURRENT_PRICES_AND_PLANS_POLICY, [released])).toBe(released);
    expect(selectV514ActiveDirectPolicy(V514_CURRENT_PRICES_AND_PLANS_POLICY, [])).toBe(V514_CURRENT_PRICES_AND_PLANS_POLICY);
  });
