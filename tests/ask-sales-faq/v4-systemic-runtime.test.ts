...
function operationalAnswerProvider(
  ...
  return providerFor(({ purpose, payload }) => {
    ...
    if (purpose === "v4_systemic_evidence_answer") {
      ...
      expect(card).toMatchObject({ authority_class: "direct_company_authority" });
        ...
        needs: [
          {
            ...
            lane: "answer",
            evidence_refs: [ref],
            answer_sentences: [{ text: card!.decision, evidence_refs: [ref] }],
            route_key: null,
            clarification_question: "",
            confidence: 0.96,
    ...
    return {
      checks: sentences.map((sentence) => ({
        ...
        answered_need_ids: validationStatus === "supported" ? validatorOverclaimsAllNeeds ? allNeedIds : [sentence.need_id] : [],
...
describe("Ask Sales V4 systemic runtime", () => {
  ...
  it("routes after semantic validation rejects every drafted sentence instead of reopening a weaker answer path", async () => {
    const result = await runAskSalesFaqV4Systemic(operationalPolicy!.question_families[0], [], {
