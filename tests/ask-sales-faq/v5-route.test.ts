...
vi.mock("@/lib/ask-sales-faq/v5-14/runtime", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@/lib/ask-sales-faq/v5-14/runtime")>()),
  runAskSalesFaqV514: mocks.run,
}));
...
import { getV514KnowledgeVersion } from "@/lib/ask-sales-faq/v5-14/knowledge";
...
describe("Ask Sales V5 isolated route", () => {
  it("reports the immutable persistence-free V5 snapshot", async () => {
    ...
    expect(response.headers.get("x-ask-sales-runtime")).toBe("v5.14-isolated");
  });
