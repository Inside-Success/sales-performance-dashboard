...
vi.mock("@/lib/ask-sales-faq/v5-7/runtime", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@/lib/ask-sales-faq/v5-7/runtime")>()),
  runAskSalesFaqV57: mocks.run,
}));
...
import { getV57KnowledgeVersion } from "@/lib/ask-sales-faq/v5-7/knowledge";
...
describe("Ask Sales V5 isolated route", () => {
  it("reports the immutable persistence-free V5 snapshot", async () => {
    ...
    expect(response.headers.get("x-ask-sales-runtime")).toBe("v5.7-isolated");
  });
