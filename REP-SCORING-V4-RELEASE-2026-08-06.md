...
- Guarded execution `446517` proved that overlap was not the only failure: the full-history coordinator scan still crashed before any worker dispatch. The source scan now reads one rotating day from the fixed July 18–August 6 launch backlog plus the newest two hours. All 19 historical shards rotate in about 9.5 hours, while current calls remain eligible every clear slot.
- Sharded execution `446722` isolated the remaining out-of-memory cause at `Read Current Score Snapshot`: 620 score outputs would have triggered the quarantine-table search 620 times. `Collapse Score Snapshot` now collapses that boundary to one item. Controlled parent `446727` succeeded in 24.2 seconds and read 263 quarantine rows once instead of multiplying them; one-call worker `446728` then succeeded in 56.5 seconds and completed its ledger outcome.
- The manager progress bar no longer divides a single rotating daily shard by that shard's completed count. It now uses the largest trusted all-window inventory as a stable launch-backlog baseline and counts unique finalized V4.2 outcomes only through that baseline's end time. This keeps the temporary catch-up percentage monotonic and prevents live-call arrivals or differently sized shards from making it move backward.
...
- The active workflow is `MM Rep Performance Reviewer V4.2 - Speaker Safe (LIVE ADAPTIVE 30-MIN SHARDED)`, active version `e3caa422-3660-41ce-89f8-23f7374e8d4d`. Runtime validation returned zero errors, and the controlled webhook remains disabled. Workflow-history version `483` is the verified hourly safe-mode rollback; version `478` remains the pre-acceleration hourly rollback.
