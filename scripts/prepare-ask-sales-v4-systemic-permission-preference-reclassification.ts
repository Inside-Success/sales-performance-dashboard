...
const permissionQuestion = /\b(?:can\s+(?:i|we|they)|may\s+(?:i|we|they)|are\s+(?:we|they)\s+allowed|is\s+it\s+(?:allowed|okay|ok|acceptable|possible)|allowed\s+to|permitted\s+to)\b/i;
const permissionWithPreference = /^\s*(?:yes|no)\b.{0,240}\b(?:try|prefer(?:red)?|if possible|when possible|ideally|default)\b/i;
const preservesPreference = /\b(?:try|prefer(?:red)?|if possible|when possible|ideally|default)\b/i;
const affected = new Set(Object.entries(previous.completed).flatMap(([sourceId, thread]) => {
  ...
  if (!source.authority_replies.some((reply) => permissionWithPreference.test(reply.text))) return [];
  const hasPreferenceDecision = (thread.decisions || []).some((decision) => preservesPreference.test(decision.decision || ""));
  return hasPreferenceDecision ? [] : [sourceId];
}));
