...
function requestedSystems() {
  ...
  const systems = argument("systems", "v5").split(",").map((value) => value.trim()).filter(Boolean);
...
function providerFailures(result: RuntimeResult) {
  return (result.runtimeMetadata?.providerAttempts || []).filter((attempt) => attempt.status !== "success").length;
}
...
async function main() {
  if (argument("diagnostic-only") !== "true") {
    throw new Error("Retained/revealed datasets require --diagnostic-only=true and cannot be used as promotion evidence");
  }
  const datasetPath = path.resolve(argument("dataset"));
  if (!argument("dataset")) throw new Error("--dataset is required");
  const outputPath = path.resolve(argument("output", "artifacts/ask-sales-faq-v5-2/retained-diagnostic.json"));
  ...
  for (const system of systems) {
    for (const conversation of report.conversations) {
      ...
      for (const prompt of conversation.prompts) {
        ...
        process.stdout.write(`${JSON.stringify({ system, conversation: conversation.id, lane: lane(result) })}\n`);
      }
    }
  }
