...
for wid,manual in [('L8Nn7xncA9ZPDdWA',False),('BMRrGxHyXMcgO6j3',True)]:
 ...
 setup='const state=structuredClone($json.state); state.coaching_version=VERSION; state.coaching_model="claude-opus-5"; const input='+('state.input' if manual else 'state.caseItem')+'; const blocks=transcriptBlocks(input.cleaned_transcript); const metadata='+('input' if manual else 'input.metadata')+';'
 ...
 if manual:
  finalCode+='const state=structuredClone(prepared.state); state.coaching_raw={...state.coaching_raw,...report}; state.provider_results.push({purpose:"structured_coaching",result:prepared.generated},{purpose:"coaching_factual_audit",result:prepared.firstAudit},{purpose:"coaching_factual_confirmation",result:$json}); return {json:{...state.input,state}};'
