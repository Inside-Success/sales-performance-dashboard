...
test('auditor can remove an invented fault without losing a supported strength',()=>{const a=audit();a.rejected_improvements=[0];a.improvement_reviews[0].verdict='reject';const r=renderCoaching(applyAudit(analysis(),a,blocks),blocks);assert.match(r.what_to_improve,/No specific sales-execution improvement/);assert.match(r.biggest_strength,/Explained an amount/)});
...
test('material-only publication omits optional polish without erasing supported strengths',()=>{const a=analysis();a.improvements[0].priority='optional';const r=renderCoaching(a,blocks,{materialOnly:true});assert.match(r.what_to_improve,/No specific sales-execution improvement/);assert.match(r.biggest_strength,/Explained an amount/);assert.equal(a.improvements.length,1);});
test('publication prefers material advice and otherwise shows one audited optional suggestion',()=>{const a=analysis();a.improvements.push({...a.improvements[0],priority:'optional',observation:'A second preference.'});let r=renderCoaching(a,blocks,{optionalFallbackOnly:true});assert.match(r.what_to_improve,/Did not reclarify/);assert.doesNotMatch(r.what_to_improve,/A second preference/);a.improvements[0].priority='optional';r=renderCoaching(a,blocks,{optionalFallbackOnly:true});assert.match(r.what_to_improve,/Optional polish:/);assert.doesNotMatch(r.what_to_improve,/A second preference/);});
test('publication blocks invented payment holds and written delivery commitments but keeps clarification',()=>{
 for(const action of ['Ask for a smaller initial commitment.','Confirm an option to keep the spot active while the buyer consults their team.','Put the timeline commitment in writing in the invoice.']){const a=analysis();a.improvements[0].better_action=action;assert.match(renderCoaching(a,blocks,{excludePolicyChanges:true}).what_to_improve,/No specific sales-execution improvement/);}
 assert.match(renderCoaching(analysis(),blocks,{excludePolicyChanges:true}).what_to_improve,/Clarify the amount/);
});

test("an empty recommendation is not a blanket compliance clearance",()=>{const a=analysis();a.improvements=[];const r=renderCoaching(a,blocks);assert.equal(r.what_to_improve,"No specific sales-execution improvement was supported by this call.");assert.doesNotMatch(r.coaching_tip,/no corrective|no mistake/i);});
...
test('reviewer can downgrade optional polish but cannot upgrade a writer priority',()=>{
 ...
 assert.equal(renderCoaching(output,blocks,{materialOnly:true}).what_id_polish,'No specific sales-execution improvement was supported by this call.');
});
