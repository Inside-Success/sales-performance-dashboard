...
type ThreadMessage = { role: "participant" | "authority"; message_ts: string };
type Corpus = { records: Array<{ source_id: string; root_ts: string; thread_messages?: ThreadMessage[] }> };
type Checkpoint = {
...
const affected = new Set(corpus.records.filter((record) =>
  record.thread_messages?.some((message) => message.role === "participant" && message.message_ts !== record.root_ts),
