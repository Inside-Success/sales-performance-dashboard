...
On 2026-07-29 the publisher preflight was made race-safe after release `kr_147f6460-8476-4c3c-8509-01c898ee17cb` was submitted eight seconds before the FAQ check completed and roughly two and a half minutes before the dashboard check completed. A queued or in-progress required check is no longer recorded immediately as a publication failure. The active publisher waits 30 seconds and rechecks the same exact PR heads, for at most 20 attempts (10 minutes). A completed unsuccessful check, invalid PR, or timeout still stops fail-closed before any merge. Existing valid PRs remain retryable from the same release row.
