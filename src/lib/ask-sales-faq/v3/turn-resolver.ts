...
const SOCIAL_ONLY = /^(?:hi|hello|hey|good (?:morning|afternoon|evening)|how are you|thanks|thank you|got it|understood|great|awesome|perfect|okay|ok)[!.?\s]*$/i;
const SOCIAL_ACK = /^(?:(?:perfect|great|awesome|okay|ok|got it|understood|thanks|thank you|appreciate it|that(?:['’]s| is) helpful)[,!?.\s-]*)+$/i;
const SOCIAL_PREFACE = /^(?:hi(?: there)?|hello|hey|good (?:morning|afternoon|evening)|thanks|thank you)(?:[!,—–-]?\s+|$)/i;
...
const GENERIC_HELP_REQUEST = /^(?:(?:hi(?: there)?|hello|hey|good (?:morning|afternoon|evening))[,!\s—–-]*)?(?:can|could|would) you (?:please )?(?:quickly )?help me(?: with)?(?: (?:another|a|a few|some|several))?(?: (?:quick|brief))?(?: sales)? questions?[!.?]*$/i;
const TOPIC_INTRO = /^(?:(?:hi(?: there)?|hello|hey|hello again|hi again|good (?:morning|afternoon|evening))[,!.\s-]*)?(?:(?:can|could|would) you (?:please )?help me (?:check|review|go over|with)(?: (?:another|a|a few|some|several))? .{2,80}(?:questions?|cases?|topics?|situations?|scenarios?)|i (?:need|want) help with .{2,80}(?:now|next)?|i have (?:another|a few|some|several) .{2,80}(?:questions?|cases?|situations?|scenarios?)|let(?:['’]s| us) (?:move|switch) to .{2,80})[!.?]*$/i;
const MEMORY_QUESTION = /\b(?:what|which)\s+(?:was|is)\s+(?:my|the)\s+(?:previous|last)\s+question\b|\bwhat did i (?:just )?ask\b/i;
const REWRITE = /\b(?:rewrite|rephrase|format|make (?:that|it)|turn (?:that|it|the previous answer|your previous answer)|put (?:that|it)|summari[sz]e|shorten|shorter|simpler language|plain english|bullet(?:s| points)?|checklist|table|answer without repeating|without repeating the route|one[- ]sentence (?:way|version)|(?:say|phrase) (?:that|it) (?:to|for) (?:the )?(?:prospect|client)|explain only what is confirmed|explain (?:that|this|it).{0,50}(?:naturally|clearly|briefly|concisely|only what)|keep (?:that|it|the answer) short|keep only what (?:i|we) need)\b/i;
const ELLIPTICAL_FOLLOW_UP = /^(?:(?:and|also|so|but)\s+)?(?:what about|how about|does that|did that|would that|could that|is that|was that|are those|do they|can they|what if|why is that|why not|then what|that means|so that means|automatically approved|.+[, ]right\??$|anything else|tell me more|(?:can you )?(?:explain|clarify|expand on|simplify) (?:that|this|it))\b/i;
