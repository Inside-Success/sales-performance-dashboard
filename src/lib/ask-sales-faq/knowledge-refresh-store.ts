...
export async function recordKnowledgeRefreshCandidates(input: {
  ...
  for (const rawCandidate of input.candidates.slice(0, 20)) {
    ...
    const insert = buildKnowledgeRefreshCandidateInsert({
      id: candidateId,
      source_id: input.sourceId,
      snapshot_id: input.snapshotId,
      snapshot_hash: input.snapshotHash,
      source_revision: sanitizeOperationalText(input.sourceRevision || "", 200) || null,
      ...
      decision_key: candidate.decisionKey || null,
      product_scopes: candidate.productScopes,
      effective_date: normalizeDate(candidate.effectiveDate),
      evidence_quotes: candidate.evidenceQuotes,
      candidate_kind: candidate.candidateKind,
      policy_domains: candidate.domains,
      policy_actions: candidate.actions,
      policy_entities: candidate.entities,
      policy_object: candidate.policyObject,
      policy_conditions: candidate.conditions,
      is_durable: candidate.isDurable,
      is_reusable: candidate.isReusable,
      answer_impact: candidate.answerImpact,
      source_authority: candidate.sourceAuthority,
      authority_name: candidate.authorityName,
      authority_basis: candidate.authorityBasis,
      atomic_decision_count: candidate.atomicDecisionCount,
      ai_model: sanitizeOperationalText(input.model, 120),
      ...
      review_note: noise.reason,
    });
    const rows = (await sql.query(
      `insert into ask_sales_faq_refresh_candidates (${insert.columnsSql})
       values (${insert.valuesSql})
