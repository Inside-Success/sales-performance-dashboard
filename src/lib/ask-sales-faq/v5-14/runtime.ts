...
function exactPolicyDecision(question: string): DirectDecision | null {
  ...
  if (/\b(?:istv|inside\s+success)\b/i.test(text) && /\b(?:price|pricing|cost)\w*\b/i.test(text) &&
    ...
    return { answer: V514_CURRENT_PRICES_AND_PLANS_POLICY.decision, policies: [V514_CURRENT_PRICES_AND_PLANS_POLICY], kind: "policy" };
  }
  if ((/\b(?:call\s*2|call\s*two|second\s+call)\b/i.test(text) ||
    ...
    return { answer: V514_CALL2_QUOTE_SEQUENCE_POLICY.decision, policies: [V514_CALL2_QUOTE_SEQUENCE_POLICY], kind: "policy" };
  }
  if (!/\bmoney\s+mondays?\b/i.test(text) &&
    ...
    return { answer: V514_WEEKLY_SUPPORT_DISCONTINUED_POLICY.decision, policies: [V514_WEEKLY_SUPPORT_DISCONTINUED_POLICY], kind: "policy" };
  }
  ...
  if (/\b(?:america['’]?s\s+(?:best|top)\s+doctors?|doctors?\s+show)\b/i.test(text) &&
    ...
    return { answer: V514_DOCTOR_NURSE_ELIGIBILITY_POLICY.decision, policies: [V514_DOCTOR_NURSE_ELIGIBILITY_POLICY], kind: "policy" };
  }
  return null;
}
