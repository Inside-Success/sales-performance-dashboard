...
async function directDeepSeekCall<T>(input: V3ProviderInput<T>, config: DirectProviderConfig): Promise<V3ProviderResult<T>> {
  const key = config.apiKey;
  if (!key) throw new V4ProviderExecutionError(`No ${config.runtimeLabel} answer provider is configured`, []);
  ...
  throw new V4ProviderExecutionError(`${config.runtimeLabel} DeepSeek request failed: ${message}`, attempts);
}

export const generateV4Json: V3Provider = async <T>(input: V3ProviderInput<T>) => {
  return directDeepSeekCall(input, {
    apiKey: process.env.ASK_SALES_V4_DEEPSEEK_API_KEY,
    model: process.env.FAQ_V4_DEEPSEEK_MODEL || DEFAULT_DIRECT_MODEL,
    userId: "ask-sales-v4-isolated",
    runtimeLabel: "V4 isolated",
  });
};

export const generateV4ValidationJson = generateV4Json;

export const generateV514ProductionJson: V3Provider = async <T>(input: V3ProviderInput<T>) => {
  return directDeepSeekCall(input, {
    apiKey: process.env.DEEPSEEK_API_KEY,
    model: process.env.FAQ_DEEPSEEK_MODEL || DEFAULT_DIRECT_MODEL,
    userId: "ask-sales-v5-14-production",
    runtimeLabel: "V5.14 production",
  });
};

export const generateV514ProductionValidationJson = generateV514ProductionJson;
