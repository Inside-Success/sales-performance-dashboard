  ...
  return (
    <Card className="magic-card border-white/80 bg-white/95">
      ...
      <CardContent className="space-y-3">
        {summary.coachingPriorities.length ? summary.coachingPriorities.map((pattern) => <PriorityCard key={pattern.key} pattern={pattern} example={examples.get(pattern.key)} />) : <div className="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-4"><div className="flex items-center gap-2 font-extrabold text-slate-900"><CheckCircle2 className="size-5 text-emerald-700" />No recurring weakness is currently supported</div><p className="mt-2 text-sm leading-6 text-slate-600">This does not mean the rep is perfect; it means the analyzed calls do not justify labeling a recurring weakness.</p></div>}
      </CardContent>
    </Card>
  );
}
