...
export default async function V7RepPage({ params }: { params: Promise<{ repKey: string }> }) {
  ...
  return <main className="magic-page"><div className="mx-auto flex max-w-6xl flex-col gap-5 px-5 pb-16 pt-8 sm:px-8">
    ...
    <Card className="magic-card bg-white"><CardContent className="p-5"><div className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Why this rep appears here</div><p className="mt-2 text-lg font-semibold leading-7 text-slate-900">{summary.reason}</p><div className="mt-4 flex items-start gap-2 rounded-xl bg-red-50 p-4 text-sm font-semibold leading-6 text-red-900"><Target className="mt-0.5 size-5 shrink-0" />{summary.action}</div></CardContent></Card>
