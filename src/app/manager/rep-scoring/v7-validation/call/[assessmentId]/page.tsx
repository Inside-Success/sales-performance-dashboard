...
export default async function V7CallPage({ params }: { params: Promise<{ assessmentId: string }> }) {
  ...
  const priorityCriteria = [...lowCriteria].sort((a, b) => statusRank(a.status) - statusRank(b.status)).slice(0, 6);
  const observedBehaviours = call.behaviours.filter((item) => item.name && item.reason).slice(0, 4);
  const showReliability = call.gradeability !== "gradable" || Boolean(call.validationWarnings.length);

  return <main className="magic-page"><div className="mx-auto flex max-w-6xl flex-col gap-5 px-5 pb-16 pt-8 sm:px-8">
    <Link prefetch href={`/manager/rep-scoring/v7-validation/rep/${encodeURIComponent(call.repEmail || call.repName)}`} className="inline-flex w-fit items-center gap-2 text-sm font-bold text-slate-600 hover:text-red-700"><ArrowLeft className="size-4" />Back to {call.repName}</Link>
    ...
    <Card className="magic-card bg-white"><CardContent className="p-5"><div className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Manager takeaway</div><p className="mt-2 text-lg font-semibold leading-7 text-slate-900">{call.mainFinding}</p>{primaryFinding?.reason ? <p className="mt-2 text-sm leading-6 text-slate-600">{primaryFinding.reason}</p> : <p className="mt-2 text-sm leading-6 text-slate-600">No material weakness was supported. Use the evidence below only if you need to audit the score.</p>}</CardContent></Card>

    <section className={`grid gap-4 ${showReliability ? "md:grid-cols-3" : "md:grid-cols-2"}`}>{showReliability ? <ContextCard label="Transcript limitation" value={humanize(call.gradeability)} detail={call.reliabilityReason} /> : null}<ContextCard label="Prospect opportunity" value={humanize(call.opportunity)} detail={call.opportunityReason} /><ContextCard label={call.callType === "Call 1" ? "Correct progression" : "Recorded outcome"} value={humanize(call.callType === "Call 1" ? call.disposition : call.outcome)} detail={call.outcomeReason || "Based on verified call facts."} /></section>
    ...
    {priorityCriteria.length ? <Card className="magic-card bg-white"><CardHeader><CardTitle>Execution checkpoints to review</CardTitle><p className="text-sm leading-6 text-slate-500">The most important below-standard checkpoints are shown first.</p></CardHeader><CardContent className="grid gap-3 md:grid-cols-2">{priorityCriteria.map((criterion) => <CriterionCard key={`${criterion.dimension}:${criterion.id}`} criterion={criterion} dimension={criterion.dimension} />)}</CardContent></Card> : null}

    {observedBehaviours.length ? <Card className="magic-card bg-white"><CardHeader><CardTitle>Observed sales behaviors</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-2">{observedBehaviours.map((item, index) => <div key={`${item.name}:${index}`} className="rounded-xl border border-slate-200 bg-slate-50/70 p-4"><div className="font-extrabold text-slate-950">{item.name}</div><p className="mt-2 text-sm leading-6 text-slate-600">{item.reason}</p>{item.evidence.slice(0, 1).map((evidence, evidenceIndex) => <Evidence key={evidenceIndex} value={evidence} />)}</div>)}</CardContent></Card> : null}
...
function statusRank(status: string) { return status === "harmful" ? 0 : status === "missed" ? 1 : status === "weak" ? 2 : status === "partial" ? 3 : 4; }
