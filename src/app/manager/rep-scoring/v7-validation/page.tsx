...
export default async function V7ValidationPage() {
  ...
  return <main className="magic-page"><div className="mx-auto flex w-full max-w-7xl flex-col gap-5 px-5 pb-16 pt-8 sm:px-8">
    <header className="magic-card magic-hero p-5 md:p-7"><div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between"><div className="max-w-3xl"><div className="mb-3 flex flex-wrap gap-2"><Badge variant="outline" className="gap-1 rounded-full border-red-100 bg-red-50 text-red-700"><ShieldCheck className="size-3.5" />Admin only</Badge><Badge variant="outline" className="rounded-full border-blue-200 bg-blue-50 text-blue-800">Isolated validation</Badge></div><h1 className="text-3xl font-extrabold text-slate-950 md:text-4xl">New scoring quality check</h1><p className="mt-3 max-w-2xl text-sm font-medium leading-6 text-slate-600">A balanced set of 150 calls was checked before any production change. The current manager dashboard and Coaching scores remain unchanged.</p></div><div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-sm text-slate-500"><Clock3 className="size-4 text-red-600" />Updated {formatDateTime(data.generatedAt)}</div></div></header>
