...
export default async function V7ValidationPage() {
  ...
  return <main className="magic-page"><div className="mx-auto flex w-full max-w-7xl flex-col gap-5 px-5 pb-16 pt-8 sm:px-8">
    <header className="magic-card magic-hero p-5 md:p-7"><div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between"><div className="max-w-3xl"><div className="mb-3 flex flex-wrap gap-2"><Badge variant="outline" className="gap-1 rounded-full border-red-100 bg-red-50 text-red-700"><ShieldCheck className="size-3.5" />Admin only</Badge><Badge variant="outline" className="rounded-full border-blue-200 bg-blue-50 text-blue-800">Isolated validation</Badge></div><h1 className="text-3xl font-extrabold text-slate-950 md:text-4xl">New scoring quality check</h1><p className="mt-3 max-w-2xl text-sm font-medium leading-6 text-slate-600">A balanced set of 100 calls is being checked before any production change. The current manager dashboard and Coaching scores remain unchanged.</p></div><div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-sm text-slate-500"><Clock3 className="size-4 text-red-600" />Updated {formatDateTime(data.generatedAt)}</div></div></header>
    ...
    <section className="grid gap-3 md:grid-cols-3"><Gate label="Balanced call coverage" passed={call1.length >= 40 && call2.length >= 40} detail={`${call1.length} Call 1 and ${call2.length} Call 2+ scored`} /><Gate label="Usable-call completion" passed={terminal >= 90} detail={`${terminal} final results; temporary failures ${retryableFailures.length}`} /><Gate label="High-score restraint" passed={scores.length >= 50 && exactHundreds / scores.length <= 0.05} detail={`${exactHundreds} of ${scores.length || 0} scores are exactly 100`} /></section>

    {needsAttention.length ? <Card className="magic-card bg-white"><CardHeader><CardTitle>Manager preview</CardTitle><p className="text-sm leading-6 text-slate-500">Only evidence-supported priorities appear. Open a rep to see the repeated issue and relevant calls.</p></CardHeader><CardContent className="grid gap-3">{needsAttention.map((rep) => <Link prefetch href={`/manager/rep-scoring/v7-validation/rep/${encodeURIComponent(rep.repEmail)}`} key={rep.repEmail} className="flex flex-col gap-3 rounded-2xl border border-red-100 bg-red-50/50 p-4 transition hover:border-red-300 md:flex-row md:items-center md:justify-between"><div><div className="font-extrabold text-slate-950">{rep.repName}</div><p className="mt-1 text-sm leading-6 text-slate-600">{rep.reason}</p><div className="mt-2 text-xs font-bold text-red-700">{rep.action}</div></div><div className="flex shrink-0 items-center gap-3"><div className="text-right"><div className="text-2xl font-extrabold text-slate-950">{rep.overallScore.toFixed(1)}</div><div className="text-xs text-slate-500">{rep.totalCalls} calls</div></div><ArrowRight className="size-5 text-red-600" /></div></Link>)}</CardContent></Card> : null}
