...
import { ASK_SALES_V514_PIPELINE_VERSION, runAskSalesFaqV514 } from "@/lib/ask-sales-faq/v5-14/runtime";
...
function reserveRequest(token: string) {
  const now = Date.now();
  const key = createHash("sha256").update(`v5:${token}`).digest("hex");
...
function json(payload: unknown, status = 200) {
  ...
  response.headers.set("x-ask-sales-runtime", ASK_SALES_V514_PIPELINE_VERSION);
  return response;
}
...
export async function GET() {
  ...
  return json({
    ...
    runtime: ASK_SALES_V514_PIPELINE_VERSION,
    persistence: false,
    ...
    governedSourcePreservationVersion: "v514-r1",
    ...
    governedOperationalAuditCount: snapshot.governedOperationalAuditReport.length,
    unresolvedPublishCollisionCount: publishCollisions.length,
...
export async function POST(request: NextRequest) {
  ...
  if (!isV4LabTokenAuthorized(labToken)) return json({ ok: false, error: "Invalid or missing isolated lab access token." }, 401);
  ...
  try {
    ...
    const knowledgeVersion = getV514KnowledgeVersion();
    ...
    if (parsed.data.historyToken) {
      try {
        ...
        if (error instanceof V4HistoryTokenError) {
          return json({ ok: false, error: "The isolated conversation history is invalid, expired, or no longer matches this knowledge snapshot. Start a new case safely." }, 409);
        }
        throw error;
      }
    }
    ...
    try {
      ...
      const result = await runAskSalesFaqV514(question, [...verifiedMessages, { role: "user", content: question }], {
      ...
      return json({ ...result, conversationId, historyToken, messageId: `v5_lab_assistant_${randomUUID()}` });
    } finally {
      reservation.release();
    }
  } catch (error) {
    console.error("Ask Sales V5.14 isolated request failed", error instanceof Error ? error.message : "unknown error");
    return json({ ok: false, error: "The isolated V5.14 runtime failed safely. No production request or database write was attempted." }, 503);
  }
}
