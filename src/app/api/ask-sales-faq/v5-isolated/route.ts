...
import {
  ASK_SALES_V56_PIPELINE_VERSION,
  runAskSalesFaqV56,
} from "@/lib/ask-sales-faq/v5-6/runtime";
...
function json(payload: unknown, status = 200) {
  ...
  response.headers.set("x-ask-sales-runtime", ASK_SALES_V56_PIPELINE_VERSION);
  return response;
}
...
export async function GET() {
  ...
  return json({
    ...
    runtime: ASK_SALES_V56_PIPELINE_VERSION,
    persistence: false,
    ...
    isolatedOwnerConfirmedOverlayCount: 1,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const knowledgeVersion = getV56KnowledgeVersion();
    ...
    try {
      ...
      const result = await runAskSalesFaqV56(question, [...verifiedMessages, { role: "user", content: question }], {
    ...
    console.error("Ask Sales V5.6 isolated request failed", error instanceof Error ? error.message : "unknown error");
    return json({ ok: false, error: "The isolated V5.6 runtime failed safely. No production request or database write was attempted." }, 503);
  }
}
